...
function toggleAuto() { 
    ...
    if(isAutoSkill) { 
        btn.classList.add('active-state'); btn.innerText = '🤖 AUTO: BẬT'; 
        if (document.getElementById('dice-container').style.display === 'flex') rollDice('normal'); 
    } else { 
        btn.classList.remove('active-state'); btn.innerText = '🤖 AUTO: TẮT'; 
    } 
}
...
// ─── HELPER: tạo img tag có scale ────────────────────────────────────────────
function _makeImgTag(src, scale) {
    const s = (scale != null ? scale : 1.0);
    const styleAttr = s !== 1.0
        ? ` style="transform: scale(${s}); transform-origin: bottom center;"`
        : '';
    return `<img src="${src}" class="char-img"${styleAttr}>`;
}

// ─── HELPER: tạo display html cho 1 nhân vật (ảnh hoặc icon fallback) ────────
function _heroDisplay(imgSrc, icon, scaleKey) {
    if (imgSrc && imgSrc !== '') {
        return _makeImgTag(imgSrc, IMAGE_SCALE[scaleKey] ?? 1.0);
    }
    return icon || '❓';
}
...
function openTeamModal() { 
    ...
    // 📐 scale main hub trong team modal
    let mainImgHtml = _heroDisplay(GAME_IMAGES.mainHub, '🗡️<br><span style="font-size:10px">MAIN</span>', 'mainHub');
...
function updateUI() {
    ...
    ACTIVE_TEAM.forEach((hero, index) => {
        if(hero) {
            ...
            if (fillBarUI && btnUI) {
                ...
                if (hero.cdCurrent >= hero.cdMax) { fillBarUI.style.background = 'var(--accent)'; btnUI.classList.add('ready'); }
                else { fillBarUI.style.background = 'var(--cd-color)'; btnUI.classList.remove('ready'); }
            }
        }
    });
    ...
    renderStatusIcons('player-sprite-container', player.statuses);
...
function renderStageMap() {
    ...
    BOARD_TILES.forEach((tile, index) => {
        ...
        if (index === player.boardPos) {
            ...
            let hpPercent = Math.max(0, (player.hp / player.maxHp) * 100);
            let hpColor = hpPercent > 50 ? 'var(--cd-color)' : (hpPercent > 20 ? '#ff9800' : 'var(--hp-color)');
            dot.innerHTML += `<div class="map-hp-bar" id="map-player-hp-wrap"><div id="map-player-hp-fill" style="width: ${hpPercent}%; background-color: ${hpColor};"></div></div>`;
        }
...
function renderActionBar() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (i < player.sequence.length) {
            let act = player.sequence[i]; slot.classList.add('filled');
        ...
        } else { slot.innerHTML = `<span style="font-size:10px; color:#5d4037;">TRỐNG</span>`; }
...
function spawnPopup(targetId, val, type, skillName = null) {
    ...
    if (targetId === 'player-sprite-container' && document.getElementById('combat-view').style.display === 'none') target = document.querySelector('.stage-dot.active');
    if(!target) return;
    const popup = document.createElement('div');
    const offsetX = (Math.random() - 0.5) * 50; const offsetY = (Math.random() - 0.5) * 30;
    if (skillName) { popup.className = 'popup-text popup-skill'; popup.innerText = skillName; }
    else { popup.className = `popup-text popup-${type}`; popup.innerText = type === 'dmg' ? `-${val}` : `+${val}`; }
    popup.style.left = `calc(50% + ${offsetX}px)`; popup.style.top = `calc(50% + ${offsetY}px)`;
...
function openShop() { if(AudioEngine && typeof AudioEngine.levelUp === 'function') AudioEngine.levelUp(); const modal = document.getElementById('shop-modal'); document.getElementById('shop-gold-display').innerText = player.gold; renderShopItems(); modal.style.display = 'flex'; }
...
function refreshShop() { if (player.gold >= 10) { player.gold -= 10; document.getElementById('shop-gold-display').innerText = player.gold; updateUI(); if(AudioEngine && typeof AudioEngine.click === 'function') AudioEngine.click(); renderShopItems(); } else { addLog("<b style='color:red'>Không đủ vàng làm mới!</b>"); } }
...
function renderShopItems() {
    ...
    shuffled.forEach(item => {
        ...
        container.innerHTML += `<div class="shop-item"><div class="shop-item-info"><div class="shop-item-icon">${item.icon}</div><div class="shop-item-details"><b>${item.name}</b><span>${item.desc}</span></div></div><button class="shop-btn-buy" ${canAfford ? '' : 'disabled'} onclick="buyShopItem('${item.id}', ${item.price})">${item.price} 🪙</button></div>`;
    });
}

function buyShopItem(id, price) {
    if (player.gold < price) return; player.gold -= price; if(AudioEngine && typeof AudioEngine.click === 'function') AudioEngine.click();
    document.getElementById('shop-gold-display').innerText = player.gold; 
    if (id === 'heal') { player.hp = Math.min(player.maxHp, player.hp + Math.floor(player.maxHp * 0.4)); spawnPopup('player-sprite-container', Math.floor(player.maxHp * 0.4), 'heal'); }
    else if (id === 'diceLow') { player.inventory.diceLow++; }
    else if (id === 'diceHigh') { player.inventory.diceHigh++; }
    else if (id === 'skill') { document.getElementById('shop-modal').style.display = 'none'; updateUI(); pendingRewardType = 'shop_event'; showRewardModal("🃏 MUA THẺ KỸ NĂNG 🃏"); return; }
    updateUI(); renderShopItems(); 
}

function showRewardModal(titleText) {
    document.getElementById('modal-title-text').innerText = titleText;
    const modal = document.getElementById('level-modal'); const choicesDiv = document.getElementById('choices'); choicesDiv.innerHTML = ''; modal.style.display = 'flex';
    ...
    if (shuffled.length === 0) {
        ...
        btn.onclick = () => { if(AudioEngine && typeof AudioEngine.click === 'function') AudioEngine.click(); player.hp = player.maxHp; executeUpgradeCallback(); };
