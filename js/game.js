function startBattle() {
    ...
    setTimeout(() => {
        ...
        window.cameraMode = 'full'; updateCamera(); showMapPhase(); 
    }, 500);
}
...
function showMapPhase() {
    ...
    window.cameraMode = 'full'; updateCamera();
    if (isAutoSkill) {
        let btnRoll = document.getElementById('btn-roll'); if(btnRoll) btnRoll.style.opacity = '0.5';
        setTimeout(() => { if (isAutoSkill && document.getElementById('dice-container').style.display === 'flex') { if(btnRoll) btnRoll.style.opacity = '1'; rollDice('normal'); } }, 1200 / gameSpeed);
    }
}
...
function addShrineBuff(percent) {
    ...
    player.maxHp = Math.floor(BASE_PLAYER_HP * (1 + player.shrineBuff));
...
async function rollDice(type = 'normal') {
    ...
    isRolling = true; if(AudioEngine && typeof AudioEngine.click === 'function') AudioEngine.click();
    ...
    if (player.lap === 25) {
        ...
        let boardWrapper = document.getElementById('monopoly-board-wrapper'); if (boardWrapper) boardWrapper.style.boxShadow = "0 0 30px red";
        let stageDots = document.getElementById('stage-dots-container'); if (stageDots) stageDots.style.background = "rgba(255,0,0,0.2)";
        for(let i=0; i<32; i++) { if(!BOARD_TILES[i].isCorner && (BOARD_TILES[i].type === 'event' || BOARD_TILES[i].type === 'empty' || BOARD_TILES[i].type === 'trap')) { BOARD_TILES[i] = { type: 'trap', icon: '🔥', name: 'Dung Nham' }; } }
        renderStageMap();
    }

    const diceUI1 = document.getElementById('dice-result-1');
    const diceUI2 = document.getElementById('dice-result-2');
    ...
    if (gameSpeed > 1) {
        ...
        if(diceUI1) diceUI1.innerText = d1; if(diceUI2) diceUI2.innerText = d2; await sleep(150); 
    } else {
        for(let i=0; i<10; i++) {
            if (type === 'low') { d1 = Math.floor(Math.random() * 3) + 1; d2 = Math.floor(Math.random() * 3) + 1; }
            else if (type === 'high') { d1 = Math.floor(Math.random() * 3) + 4; d2 = Math.floor(Math.random() * 3) + 4; }
    ...
    window.cameraMode = 'focus'; window.currentCamScale = 0.6; updateCamera(); await sleep(400 / gameSpeed);
    ...
    for(let step = 0; step < totalSteps; step++) {
        ...
        if(player.boardPos >= BOARD_TILES.length) { 
            ...
            for(let j=0; j<32; j++) { if(!BOARD_TILES[j].isCorner) BOARD_TILES[j] = generateRandomTile(); }
        ...
        if (isFinalStep) {
            window.currentCamScale = 0.7; renderStageMap(); 
            ...
            window.currentCamScale = 0.68; updateCamera();
            await sleep(220 / gameSpeed);
        } else {
            window.currentCamScale = 0.6 + (progress * 0.05);
            window.currentCamShakeX = 0; window.currentCamShakeY = 0;
            renderStageMap(); 
    ...
    window.cameraMode = 'full'; window.currentCamShakeX = 0; window.currentCamShakeY = 0;
    updateCamera(); await sleep(400 / gameSpeed); 

    if (passedStart) {
        addLog(`<b style="color:green">Đi ngang Trại: +20% HP, +50 Vàng!</b>`);
        let heal = Math.floor(player.maxHp * 0.2);
        player.hp = Math.min(player.maxHp, player.hp + heal); player.gold += 50; 
    ...
    if ((passedStart || player.boardPos === 0) && player.lap % 2 === 0 && !isAutoSkill) {
        addLog(`<b style="color:#ffcc00">🛒 Thương Nhân đã xuất hiện!</b>`); openShop();
    } else { handleTileEvent(); }
}

async function handleTileEvent() {
    const pos = player.boardPos; const tile = BOARD_TILES[pos];
    ...
    if (!tile.isCorner && player.shrines[pos] > 0) {
        if (player.shrines[pos] === 1) {
            ...
            player.shrines[pos] = 3; addShrineBuff(0.08); renderStageMap(); pendingRewardType = 'event'; 
            ...
            showRewardModal("🕍 QUÀ ĐỀN THỜ TỐI THƯỢNG 🕍"); return; 
        ...
        player.shrines[pos] = 1; addShrineBuff(0.01);
        spawnPopup('player-sprite-container', 0, 'skill', '⛩️ +1% STATS'); renderStageMap(); await sleep(800); showMapPhase(); return; 
    }
    ...
    if (tile.type === 'start') {
        ...
        else if (r < 90) { addLog("<b style='color:green'>Gacha: Trúng Thẻ Kỹ Năng!</b>"); pendingRewardType = 'event'; if (isAutoSkill) { selectUpgrade(Object.keys(ACTIONS_LIB)[Math.floor(Math.random()*4)]); return; } showRewardModal("🎰 TẾ ĐÀN: NHẬN KỸ NĂNG 🎰"); }
    ...
    else if (tile.type === 'boss_gate') { if(player.lap >= 25 || isAutoSkill) { chooseFightBoss(); } else { document.getElementById('boss-gate-modal').style.display = 'flex'; } }
    else if (tile.type === 'empty') { showMapPhase(); }
    else if (tile.type === 'trap') {
        addLog("<b style='color:red'>Dẫm phải Cạm Bẫy/Dung Nham, mất 15 HP!</b>");
        player.hp -= 15; spawnPopup('player-sprite-container', 15, 'dmg'); triggerShake('player-sprite-container'); updateUI();
        await sleep(1000);
        if (player.hp <= 0) { addLog("<b style='color:red'>Bạn đã gục ngã!</b>"); setTimeout(() => { alert("GAME OVER."); location.reload(); }, 1000); return; }
        showMapPhase();
    } 
    else if (tile.type === 'forge') {
        addLog("<b style='color:orange'>Đến Lò Rèn: Nâng cấp ngẫu nhiên 1 Kỹ năng!</b>");
        if(player.sequence.length > 0) { let randSkill = player.sequence[Math.floor(Math.random() * player.sequence.length)]; randSkill.level += 1; renderActionBar(); }
        await sleep(1000); showMapPhase();
    }
    else if (tile.type === 'event') {
        addLog("Bất ngờ nhặt được thẻ kỹ năng!");
        pendingRewardType = 'event';
        if (isAutoSkill) { selectUpgrade(Object.keys(ACTIONS_LIB)[Math.floor(Math.random()*4)]); return; }
...
function spawnNextStage(tileType) {
    ...
    if (monsterSprite) { monsterSprite.style.display = 'flex'; monsterSprite.style.opacity = '1'; monsterSprite.style.transform = 'scale(1)'; }

    monster.statuses = { poison: 0, stun: 0, slow: 0, weaken: 0 };
    monster.currentTileType = tileType;
    ...
    // 📐 Set Main Idle với scale
    let battleMainIcon = document.getElementById('battle-main-icon');
    if (battleMainIcon) {
        battleMainIcon.innerHTML = GAME_IMAGES.mainIdle !== ''
            ? _makeImgTag(GAME_IMAGES.mainIdle, IMAGE_SCALE.mainIdle ?? 1.0)
            : '🗡️';
    }

    let monsterImageLink = ''; 
    if (tileType === 'chest') {
        monster.maxHp = 1; monster.hp = 1; monster.atk = 0; monster.icon = '🎁'; monsterImageLink = GAME_IMAGES.chest; 
        ...
        const isBoss = (tileType === 'boss'); if (atkValElement) atkValElement.style.display = "inline";
        const hpGrowthMultiplier = 1 + (player.lap * 0.30);
        const atkGrowthMultiplier = 1 + (player.lap * 0.15); 
        ...
        if (isBoss) { 
            ...
            monsterImageLink = GAME_IMAGES.bossIdle; 
        } else { monsterImageLink = GAME_IMAGES.monsterIdle; }
    ...
    if (mIconUI) {
        if (monsterImageLink !== '') { mIconUI.innerHTML = `<img src="${monsterImageLink}" class="char-img">`; }
        else { mIconUI.innerText = monster.icon; }
    }
...
async function executeSupportSkill(index) {
    ...
    if (entityEl) { 
        entityEl.style.transform = `${currentTransform} translateY(-30px) rotate(15deg) scale(1.2)`; 
        // 📐 ĐỔI ẢNH SKILL (với scale)
        if(hero.imgSkill !== '') {
            let imgIcon = entityEl.querySelector('.support-icon');
            if(imgIcon) imgIcon.innerHTML = _makeImgTag(hero.imgSkill, IMAGE_SCALE[hero.id] ?? 1.0);
        }
    }
    ...
    setTimeout(() => { 
        if(entityEl) { 
            entityEl.style.transform = currentTransform; 
            // 📐 TRẢ VỀ ẢNH IDLE (với scale)
            let imgIcon = entityEl.querySelector('.support-icon');
            let displayChar = _heroDisplay(hero.imgIdle, hero.icon, hero.id);
            if(imgIcon) imgIcon.innerHTML = displayChar;
        } 
    }, 600);
}
...
async function chargeSupportMana() {
    ...
    for (let j = 0; j < ACTIVE_TEAM.length; j++) { 
        ...
        if (hero) {
            if (typeof hero.passive === 'function') { hero.passive(player); updateUI(); }
            if (hero.cdCurrent < hero.cdMax) { hero.cdCurrent++; updateUI(); }
            if (isAutoSkill && hero.cdCurrent >= hero.cdMax && monster.hp > 0) {
                await sleep(150); await executeSupportSkill(j); triggered = true;
            }
        } 
    }
...
async function gameLoop() {
    if (player.hp <= 0) {
        ...
        setTimeout(() => { alert("Tải lại trang để chơi lại."); location.reload(); }, 1000); return;
    }
    const type = monster.currentTileType || 'monster';

    let pLogs = processStatuses(player, 'player-sprite-container');
    let mLogs = processStatuses(monster, 'monster-sprite');
    pLogs.forEach(l => addLog(l)); mLogs.forEach(l => addLog(l));
    ...
    if (isAutoSkill) { await chargeSupportMana(); if (monster.hp <= 0) { await handleVictory(type); return; } }
    
    let chariotActive = player._chariotBuff;
    if (chariotActive) {
        player._chariotBuff = false;
        addLog(`<b style="color:#ffcc00">⚡ [The Chariot]</b> Buff x2 ATK đang kích hoạt!`);
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const slot = document.getElementById(`slot-${i}`); if(slot) slot.classList.add('active'); 

        if (player.statuses.stun > 0) {
            addLog(`Vũ khí chính đang bị <b style="color:orange">Choáng</b>, mất lượt!`);
            await sleep(400); if(slot) slot.classList.remove('active'); continue;
        }

        const actionObj = player.sequence[i]; const action = ACTIONS_LIB[actionObj.id]; 
        if (!action) { if(slot) slot.classList.remove('active'); continue; }
        ...
        if (mainIcon) {
            mainIcon.style.transform = `${mainCurrentTransform} translateY(-30px) rotate(15deg) scale(1.2)`; 
            // 📐 ĐỔI ẢNH ATK (với scale)
            if(GAME_IMAGES.mainAtk !== '') mainIcon.innerHTML = _makeImgTag(GAME_IMAGES.mainAtk, IMAGE_SCALE.mainAtk ?? 1.0);
        }
        
        await sleep(150);
        if (monster.hp > 0) {
            if(AudioEngine && typeof AudioEngine.attack === 'function') AudioEngine.attack(); 
            let baseDmg = action.dmg || 0; let baseHeal = action.heal || 0;
            let mainDmg = type === 'chest' ? 1 : Math.floor((baseDmg * skillLevel) * dmgMultiplier * shrineMultiplier * elaineMult * chariotMult);
            
            if (mainDmg > 0) { 
                monster.hp -= mainDmg; spawnPopup('monster-sprite', mainDmg, 'dmg'); 
        ...
        updateUI(); 
        if (mainIcon) {
            mainIcon.style.transform = mainCurrentTransform;
            // 📐 TRẢ VỀ IDLE (với scale)
            mainIcon.innerHTML = GAME_IMAGES.mainIdle !== ''
                ? _makeImgTag(GAME_IMAGES.mainIdle, IMAGE_SCALE.mainIdle ?? 1.0)
                : '🗡️';
        }
        await sleep(200);

        // ── 2. SUPPORT LAO LÊN ĐÁNH BỒI ─────────────────────────────────────
        if (isAllAttack && type !== 'chest' && monster.hp > 0) {
            for (let j = 0; j < ACTIVE_TEAM.length; j++) {
                ...
                if (hero) {
                    ...
                    if (suppEl) {
                        suppEl.style.transform = `${suppCurrentTransform} translateY(-30px) rotate(15deg) scale(1.2)`;
                        // 📐 ĐỔI ẢNH ATK SUPPORT (với scale)
                        if(hero.imgAtk !== '') {
                            let imgIcon = suppEl.querySelector('.support-icon');
                            if(imgIcon) imgIcon.innerHTML = _makeImgTag(hero.imgAtk, IMAGE_SCALE[hero.id] ?? 1.0);
                        }
                    }
                    ...
                    if (suppDmg > 0) {
                        monster.hp -= suppDmg; spawnPopup('monster-sprite', suppDmg, 'dmg'); triggerShake('monster-sprite');
                        addLog(`> <b style="color:var(--accent)">[Combo]</b> ${hero.name} bồi thêm <b style="color:#ff4d4d">${suppDmg}</b> sát thương!`);
                    }

                    if (hero.id === 'tarot' && typeof hero.passiveProc === 'function' && monster.hp > 0) hero.passiveProc(monster);

                    updateUI();
                    if (suppEl) {
                        suppEl.style.transform = suppCurrentTransform;
                        // 📐 TRẢ VỀ IDLE SUPPORT (với scale)
                        let imgIcon = suppEl.querySelector('.support-icon');
                        if(imgIcon) imgIcon.innerHTML = _heroDisplay(hero.imgIdle, hero.icon, hero.id);
                    }
                    await sleep(200);
                    if (monster.hp <= 0) break;
                }
        ...
        await chargeSupportMana(); if(slot) slot.classList.remove('active'); await sleep(200); if (monster.hp <= 0) break;
    }
    ...
    if (type !== 'chest') { 
        ...
        if (monster.statuses.stun > 0) { 
            addLog(`Quái vật đang bị <b style="color:orange">Choáng</b>, mất lượt phản công!`); await sleep(500); 
        } else {
            ...
            let atkImg = isBoss ? GAME_IMAGES.bossAtk : GAME_IMAGES.monsterAtk;
            if(mIconUI && atkImg !== '') mIconUI.innerHTML = `<img src="${atkImg}" class="char-img">`;
            ...
            if (player._starShield) {
                ...
                addLog(`<b style="color:#4da3ff">🌟 [The Star]</b> Giáp Ngôi Sao hấp thụ hoàn toàn đòn tấn công ${finalMAtk} DMG!`);
            ...
            let idleImg = isBoss ? GAME_IMAGES.bossIdle : GAME_IMAGES.monsterIdle;
            if(mIconUI && idleImg !== '') mIconUI.innerHTML = `<img src="${idleImg}" class="char-img">`;
            await sleep(200);
        }
...
async function showStageClearAnim(text) {
    ...
    if(banner) { banner.innerText = text; banner.classList.add('show'); }
    await sleep(1500);
...
async function handleVictory(stageType) {
    let mSprite = document.getElementById('monster-sprite'); if (mSprite) mSprite.style.opacity = '0'; 
    ...
    if (stageType === 'chest') {
        ...
        if (isAutoSkill) { selectUpgrade(Object.keys(ACTIONS_LIB)[Math.floor(Math.random()*Object.keys(ACTIONS_LIB).length)]); return; }
    ...
    if (stageType === 'boss') {
        ...
        pendingRewardType = 'boss'; player.exp += 50; updateUI(); await showStageClearAnim("BOSS BỊ TIÊU DIỆT!"); 
        if (isAutoSkill) { selectUpgrade(Object.keys(ACTIONS_LIB)[Math.floor(Math.random()*Object.keys(ACTIONS_LIB).length)]); return; }
    ...
    if (player.exp >= player.nextExp) {
        if(AudioEngine && typeof AudioEngine.levelUp === 'function') AudioEngine.levelUp();
        pendingRewardType = 'level'; await showStageClearAnim("LEVEL UP!"); 
        if (isAutoSkill) { selectUpgrade(Object.keys(ACTIONS_LIB)[Math.floor(Math.random()*Object.keys(ACTIONS_LIB).length)]); return; }
...
function executeUpgradeCallback() {
    ...
    let lvlModal = document.getElementById('level-modal'); if(lvlModal) lvlModal.style.display = 'none';
    renderActionBar(); updateUI(); showMapPhase();
}